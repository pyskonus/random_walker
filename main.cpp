...
void main_alg(Eigen::VectorXd& X, const std::vector<std::pair<unsigned, unsigned>>& order,
    ...
    for (unsigned i = 0; i < b.size(); ++i) {
        b.coeffRef(i) = b_entry(i+seeds.size(), order, std::pair{wrapper.height, wrapper.width}, seeds, 0, wrapper);
    }
...
void form_final_img(PNG& image, const PNG& mask, const Eigen::VectorXd& X, unsigned ss,
    ...
    for (unsigned y = 0; y < image.height; ++y) {
        for (unsigned x = 0; x < image.width; ++x) {
            if (mask.R.coeffRef(x,y) == 0) {    /// definitely background
                image.R.coeffRef(x,y) = 0;
                image.G.coeffRef(x,y) = 0;
                image.B.coeffRef(x,y) = 0;
            }
        }
    }
    /// set unseeded
    for (unsigned x_idx = 0; x_idx < X.size(); ++x_idx) {
        if (X.coeffRef(x_idx) > 0.5) {
            image.R.coeffRef(order[x_idx+ss].first, order[x_idx+ss].second) = 0;
            image.G.coeffRef(order[x_idx+ss].first, order[x_idx+ss].second) = 0;
            image.B.coeffRef(order[x_idx+ss].first, order[x_idx+ss].second) = 0;
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    for (unsigned y = 0; y < image.height; ++y)
    {
        for (unsigned x = 0; x < image.width; ++x)
        {
            if (image.R.coeffRef(x,y) == 1) /// white
            {
                for (const auto& neighbour: adjacent_nodes(std::pair<unsigned, unsigned>{x,y},
                                                           std::pair{image.height, image.width}))
                {
                    if (image.R.coeffRef(neighbour.first,neighbour.second) != 0 && image.R.coeffRef(neighbour.first,neighbour.second) != 1)
                        seeds[std::pair<unsigned, unsigned>{x,y}] = 0;  /// 0 means it's the thing
                }
            } else if (image.R.coeffRef(x,y) == 0)  /// black
            {
                for (const auto& neighbour: adjacent_nodes(std::pair<unsigned, unsigned>{x,y},
                                                           std::pair{image.height, image.width}))
                {
